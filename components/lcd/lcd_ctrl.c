...
void init_lcd()
{
    ...
    static lv_disp_drv_t
        disp_drv;   // contains callback functions to interact with the display
    ESP_LOGI(TAG, "Initializing LCD controller");
    ...
    ESP_LOGI(TAG, "Initializing SPI master for LCD");
    ...
    // 1/10 screen sized
    lv_color_t* buf1 =
        heap_caps_malloc(EXAMPLE_LCD_H_RES * 20 * sizeof(lv_color_t),
                         MALLOC_CAP_DMA);
    assert(buf1);
    lv_color_t* buf2 =
        heap_caps_malloc(EXAMPLE_LCD_H_RES * 20 * sizeof(lv_color_t),
